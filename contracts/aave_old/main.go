...
func main() {
	...
	query := ethereum.FilterQuery{
		Topics:    [][]common.Hash{{common.HexToHash("0xde6857219544bb5b7746f48ed30be6386fefc61b2f864cacf559893bf50fd951")}},
	...
	for {
		...
		case vLog := <-logs:
			...
			// fbalance.SetString(event.LiquidatedCollateralAmount.String())
			// newVal := new(big.Float).Quo(fbalance, big.NewFloat(math.Pow10(18)))
			// fmt.Println("new amt: ", newVal)
			fmt.Println("raw", event)
			// fmt.Println("asset:", event.CollateralAsset.String())
			// instance, _ := NewToken(event.CollateralAsset, client)
			// name, err := instance.Name(&bind.CallOpts{})
			// if err != nil {
			// 	log.Fatal("contract name", err)
			// }
			// fmt.Println("coin name:", name)
			// fmt.Println("raw", event)
			// sendTweet(newVal, name, vLog.TxHash.Hex())
		}
	}
